        ...
        <div id="lz-workshop-modal" class="lz-modal">
            ...
            <div class="lz-workshop-header">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                    <h3 style="margin:0;" data-i18n="localyze.workshop.title">Location Workshop</h3>
                    <div style="font-size:0.85em; opacity:0.6;"><span data-i18n="localyze.workshop.label_session">Session:</span> ${escapeHtml(sessionId)}</div>
                </div>
                <div class="lz-tab-bar">
                    <button class="lz-tab-btn menu_button" data-tab="library" data-i18n="localyze.workshop.tab_library">Library</button>
                    <button class="lz-tab-btn menu_button" data-tab="architect" data-i18n="localyze.workshop.tab_architect">Architect</button>
                    <button class="lz-tab-btn menu_button" data-tab="explorer" data-i18n="localyze.workshop.tab_explorer">Explorer</button>
                </div>
            </div>
            ...
            <div class="lz-workshop-body">
                ...
                <div id="lz-tab-library" class="lz-tab-panel lz-hidden">
                    <p style="margin-top:0; opacity:0.7; font-size:0.9em; flex-shrink:0;" data-i18n="localyze.workshop.library_hint">
            ...
            <div class="lz-workshop-controls">
                <button id="lz-workshop-close" class="menu_button" data-i18n="localyze.workshop.btn_cancel">Cancel</button>
                <button id="lz-arch-finalize" class="menu_button menu_button_success" data-i18n="localyze.workshop.btn_finalize">Apply and Finalize</button>
            </div>
        </div>
    </div>`;
}
...
export function getLibraryListHTML(drafts, currentKey) {
    if (drafts.length === 0) {
        ...
        <div style="text-align:center; padding:40px; opacity:0.5;">
            ...
            <span data-i18n="localyze.workshop.library_empty">Library is empty. Use "Explorer" to discover new places.</span>
        </div>`;
    }
    ...
    return drafts.map(([key, loc]) => {
        ...
        <div class="lz-library-item ${isCurrent ? 'lz-active-loc' : ''}" data-key="${escapeHtml(key)}">
            ...
            <div class="lz-lib-actions">
                <i class="fa-solid fa-location-arrow lz-lib-apply" data-i18n="[title]localyze.workshop.apply_title" title="Apply Location"></i>
                <i class="fa-solid fa-pen-to-square lz-lib-edit" data-i18n="[title]localyze.workshop.edit_title" title="Edit in Architect"></i>
                <i class="fa-solid fa-trash lz-lib-delete" data-i18n="[title]localyze.workshop.delete_title" title="Delete Location"></i>
            </div>
        </div>`;
    }).join('');
}
...
export function getArchitectGridHTML(draft, currentImgUrl, proposedImgUrl, proposedLabel = 'Proposed') {
    ...
    <div class="lz-architect-fields">
        <label data-i18n="localyze.workshop.label_name">Location Name</label>
        <input type="text" id="lz-arch-name" class="text_pole" style="width:100%;" value="${escapeHtml(draft.name)}" />

        <label><span data-i18n="localyze.workshop.label_definition">Definition (Logic)</span> <i class="fa-solid fa-wand-sparkles lz-regen-spark" data-field="description" data-i18n="[title]localyze.workshop.regen_logic_title" title="Regenerate logic from context"></i></label>
        ...
        <label><span data-i18n="localyze.workshop.label_visuals">Visuals (Image Prompt)</span> <i class="fa-solid fa-wand-sparkles lz-regen-spark" data-field="imagePrompt" data-i18n="[title]localyze.workshop.regen_prompt_title" title="Regenerate prompt from definition"></i></label>
        ...
        <div class="lz-architect-actions">
            <button id="lz-arch-preview-btn" class="menu_button" data-i18n="localyze.workshop.btn_preview">Thumbnail Preview</button>
    ...
    <div class="lz-preview-pane">
        <div class="lz-preview-box">
            <small data-i18n="localyze.workshop.label_current_bg">Current Background</small>
...
export function getArchitectEmptyHTML() {
    ...
    <div id="lz-arch-empty" style="text-align:center; padding:60px; opacity:0.5;">
        ...
        <span data-i18n="localyze.workshop.architect_empty">Select a location from the Library or use the Explorer to start refining.</span>
    </div>`;
}
...
export function getExplorerHTML() {
    ...
    <div style="margin-bottom:20px; flex-shrink:0;">
        <h4 data-i18n="localyze.workshop.explorer_title">Automated Discovery</h4>
        <p style="opacity:0.8;" data-i18n="localyze.workshop.explorer_hint">Analyze the recent turns of the conversation to identify a new location. You can provide keywords to guide the AI's imagination.</p>
        <input type="text" id="lz-explorer-keywords" class="text_pole" data-i18n="[placeholder]localyze.workshop.explorer_placeholder" placeholder="Optional keywords (e.g. 'A dark damp cave')..." style="width:100%;" />
    </div>
    <button id="lz-explorer-go" class="menu_button" style="width:100%; padding: 12px; flex-shrink:0;" data-i18n="localyze.workshop.btn_discover">Discover Location</button>
    <div id="lz-explorer-status" class="lz-hidden" style="margin-top:25px; text-align:center; opacity:0.7;">
        <i class="fa-solid fa-spinner fa-spin" style="font-size:1.5em; margin-bottom:10px;"></i><br/><span data-i18n="localyze.workshop.explorer_status">Analyzing roleplay context...</span>
    </div>`;
}