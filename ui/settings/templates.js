...
export function buildCallRow(id, label, promptKey, profileKey, historyKey = null, guidance = '', i18nBase = '') {
    ...
    const historyRow = historyKey ? `
        <div style="display:flex;align-items:center;gap:8px;margin-top:8px;">
            <label style="font-size:0.85em;opacity:0.75;white-space:nowrap;" data-i18n="localyze.settings.label_history">History:</label>
            <input id="lz-history-${safeId}" type="number" min="0" step="1"
                ...
                style="width:60px;" />
            <span style="font-size:0.83em;opacity:0.6;" data-i18n="localyze.settings.label_pairs">pairs (0 = off)</span>
        </div>` : '';
    ...
    <div class="lz-call-row" style="margin-bottom:16px;padding:12px;border:1px solid var(--SmartThemeBorderColor,#555);border-radius:6px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
            ...
            <button class="menu_button lz-open-prompt" data-prompt-key="${safePromptKey}" 
                data-i18n="localyze.settings.btn_edit_prompt"
        ...
        <div class="lz-profile-row" style="display:flex;align-items:center;gap:8px;">
            <label style="font-size:0.85em;opacity:0.75;white-space:nowrap;" data-i18n="localyze.settings.label_connection">Connection:</label>
    ...
    <div id="lz-settings" class="extension_settings">
        <div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b><i class="fa-solid fa-location-dot"></i> <span data-i18n="localyze.settings.header">Localyze</span></b>
            ...
            <div class="inline-drawer-content">
                ...
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid var(--SmartThemeBorderColor,#444);">
                    <label class="checkbox_label" style="font-size:0.85em;cursor:pointer;">
                        ...
                        <span data-i18n="localyze.settings.parallax_label">Parallax backgrounds</span>
                    </label>
                    <span style="font-size:0.78em;opacity:0.55;" data-i18n="localyze.settings.parallax_hint">Pans wide images horizontally with mouse or tilt on narrow screens</span>
                </div>

                <!-- Profile Management Bar -->
                <div class="lz-profile-bar" style="display:flex;align-items:center;gap:4px;margin-bottom:12px;">
                    <select id="lz-profile-select" class="text_pole" style="flex:1;" data-i18n="[title]localyze.settings.profile_select_title" title="Active settings profile"></select>
                    <button id="lz-profile-save" class="menu_button" data-i18n="[title]localyze.settings.btn_save_profile" title="Save profile" style="padding:2px 8px;">&#x1F4BE;</button>
                    <button id="lz-profile-add" class="menu_button" data-i18n="[title]localyze.settings.btn_add_profile" title="New profile" style="padding:2px 8px;">&#x2795;</button>
                    <button id="lz-profile-rename" class="menu_button" data-i18n="[title]localyze.settings.btn_rename_profile" title="Rename profile" style="padding:2px 8px;">&#x270F;&#xFE0F;</button>
                    <button id="lz-profile-delete" class="menu_button" data-i18n="[title]localyze.settings.btn_delete_profile" title="Delete profile" style="padding:2px 8px;">&#x1F5D1;&#xFE0F;</button>
                </div>
                
                <p style="font-size:0.85em;opacity:0.7;margin:0 0 14px;" data-i18n="localyze.settings.profile_hint">
                ...
                ${buildCallRow('boolean',    'Step 1 — Location Changed? (Boolean)',   'booleanPrompt',    'booleanProfileId',    'booleanHistory', step1Guidance, 'localyze.settings.step1')}
                ${buildCallRow('classifier', 'Step 2 — Which Location? (Classifier)', 'classifierPrompt', 'classifierProfileId', 'classifierHistory', creativeGuidance, 'localyze.settings.step2')}
                ${buildCallRow('describer',  'Step 3 — Describe New Location',        'describerPrompt',  'describerProfileId',  'describerHistory', creativeGuidance, 'localyze.settings.step3')}
                ...
                <div style="margin: -8px 0 16px 12px; display: flex; flex-direction: column; gap: 4px;">
                    <label class="checkbox_label" style="font-size: 0.82em; cursor: pointer; opacity: 0.85;">
                        ...
                        <span data-i18n="localyze.settings.step3.auto_accept_location">Auto-Accept Location (Skip popup)</span>
                    </label>
                    <label class="checkbox_label" style="font-size: 0.82em; cursor: pointer; opacity: 0.85;">
                        <input type="checkbox" id="lz-auto-accept-description" />
                        <span data-i18n="localyze.settings.step3.auto_accept_description">Auto-Accept Description (Skip Architect review)</span>
                    </label>
                </div>

                ${buildCallRow('discovery',  'Step 4 — Targeted Discovery',            'discoveryPrompt',  'discoveryProfileId',  'discoveryHistory', creativeGuidance, 'localyze.settings.step4')}
                
                <!-- Image Generation Section -->
                <div style="margin-top:18px;padding-top:14px;border-top:1px solid var(--SmartThemeBorderColor,#444);">
                    <strong style="font-size:0.95em;" data-i18n="localyze.settings.image_gen_header">Image Generation</strong>
                    <p style="font-size:0.83em;opacity:0.65;margin:4px 0 12px;" data-i18n="localyze.settings.image_gen_hint">
                        Images are generated via Pollinations. Enter your API key to securely save it to your server vault.<br/>
                        <strong style="color:var(--SmartThemeWarningColor,#ffc107);" data-i18n="localyze.settings.image_gen_warning">Note:</strong> Extensions require <code>allowKeysExposure: true</code> in <code>config.yaml</code> to read this key.
                    </p>

                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                        <label style="font-size:0.85em;opacity:0.75;white-space:nowrap;min-width:80px;" data-i18n="localyze.settings.label_api_key">API Key:</label>
                        <input type="password" id="lz-pollinations-key" class="text_pole" data-i18n="[placeholder]localyze.settings.placeholder_api_key" placeholder="Enter new sk_ key..." style="flex:1;" />
                        <button class="menu_button" id="lz-pollinations-save" style="white-space:nowrap;" data-i18n="localyze.settings.btn_save_vault">Save to Vault</button>
                    </div>
                    <div id="lz-key-status-indicator" style="font-size:0.82em; margin-left:88px; margin-bottom:12px;">
                        <span style="opacity:0.6;"><i class="fa-solid fa-spinner fa-spin"></i> <span data-i18n="localyze.settings.status_checking_vault">Checking vault...</span></span>
                    </div>

                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                        <button class="menu_button" id="lz-pollinations-check" data-i18n="localyze.settings.btn_test_connection">Test Connection</button>
                    </div>
                    <div id="lz-pollinations-status" style="font-size:0.82em;opacity:0.65;margin-bottom:8px;"></div>

                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                        <label style="font-size:0.85em;opacity:0.75;white-space:nowrap;min-width:80px;" data-i18n="localyze.settings.label_model">Model:</label>
                    ...
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                        <label style="font-size:0.85em;opacity:0.75;white-space:nowrap;min-width:80px;" data-i18n="localyze.settings.label_prompt_template">Prompt:</label>
                        <button class="menu_button lz-open-prompt" data-prompt-key="imagePromptTemplate"
                            data-i18n="localyze.settings.btn_edit_template"
                    ...
                    <div style="display:flex;align-items:center;gap:8px;">
                        <label class="checkbox_label" style="font-size:0.85em;cursor:pointer;">
                            ...
                            <span data-i18n="localyze.settings.label_dev_mode">Dev mode</span>
                        </label>
                        <span style="font-size:0.78em;opacity:0.55;" data-i18n="localyze.settings.dev_mode_hint">Generates 320×180 preview images to save credits</span>
                    </div>
                </div>

                <!-- Maintenance -->
                <div style="margin-top:18px;padding-top:14px;border-top:1px solid var(--SmartThemeBorderColor,#444);">
                    <strong style="font-size:0.95em;" data-i18n="localyze.settings.maintenance_header">Maintenance</strong>
                    <p style="font-size:0.83em;opacity:0.65;margin:4px 0 12px;" data-i18n="localyze.settings.maintenance_hint">
                        Scan for background images no longer referenced by any location.
                    </p>
                    <div style="display:flex;align-items:center;gap:8px;">
                        <button class="menu_button" id="lz-audit-btn">
                            <i class="fa-solid fa-trash-can"></i> <span data-i18n="localyze.settings.btn_audit_images">Audit Images</span>
                        </button>
